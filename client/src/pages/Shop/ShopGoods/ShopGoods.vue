      ...
      <div class="menu-wrapper">
        <ul>
          <li class="menu-item" v-for="(good, index) in goods" :key="index"
              :class="{current: index === currentIndex}" @click="tabMenu(index)" ref="menuList">
...
<script>
  ...
  export default {
    data () {
      return {
        scrollY: 0,//食品展示列表的滚动高度
        tops: [],  //每个食品分类展示列表距离顶部的高度的数组
        food: {}   // 需要显示的food
      }
    },
    mounted() {
    ...
    methods: {
      ...
      _initScroll () {
        //左边分类导航滚动
        this.leftBscroll = new BetterScroll('.menu-wrapper', {
          click: true
        })
        //右侧食品列表滚动
      ...
      //显示某个食品
      showFood (food) {
        this.food = food //设置需要传递的数据
        this.$refs.food.toggleShow()  //调用自组件方法，实现显示隐藏
      }
    },
    components: {
      CartControl,
